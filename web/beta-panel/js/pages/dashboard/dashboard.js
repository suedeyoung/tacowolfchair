	...
	socket.getDBValues('panel_get_settings', {
		...
		socket.getDBValue('dashboard_get_events', 'panelData', 'data', function(e) {
			...
			socket.getDBValue('dashboard_get_data', 'panelData', 'stream', function(e) {
				...
				// Set communities.
				if (e.communities.length > 0) {
					let c = e.communities.split(',');
					for (let i = 0; i < c.length; i++) {
						$('#stream-community-' + (i + 1)).val(c[i]);
					}
				}
				// Set views if not hidden.
				helpers.handlePanelSetInfo($('#dashboard-views').data('number', helpers.parseNumber(e.views)), 'dashboard-views');
				// Set viewers.
				helpers.handlePanelSetInfo($('#dashboard-viewers').data('number', helpers.parseNumber(e.viewers)), 'dashboard-viewers');
				// Set followers.
				helpers.handlePanelSetInfo($('#dashboard-followers').data('number', helpers.parseNumber(e.followers)), 'dashboard-followers');
				// Set uptime.
				if (e.isLive) {
...
$(function() {
    ...
    // Handle updating the title, game, and communities.
    $('#dashboard-btn-update').on('click', function() {
    	...
    	socket.sendCommand('update_title', 'settitlesilent ' + $('#stream-title').val(), function() {
    		...
    		socket.sendCommand('update_game', 'setgamesilent ' + $('#stream-game').val(), function() {
    			...
    			for (let i = 1; i <= 3; i++) {
    				let c = $('#stream-community-' + i);

    				if (c.val().length > 0) {
    					communities.push(c.val().toLowerCase());
    				}
    			}

    			// Update communities.
    			socket.sendCommand('update_community', 'setcommunitysilent ' + communities.join(', '), function() {
    ...
    // Dark mode toggle.
    $('#dark-mode-toggle').off().on('click', function() {
    	// Update the toggle.
